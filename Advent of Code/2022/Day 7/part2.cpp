...
int main(int argc, char** argv){
    ...
    int reduceAmount = totalSize - 40000000;
    int smallestAbove = totalSize;
    std::cout << totalSize << "\n";
    std::cout << reduceAmount << "\n";
    for(auto itr = allDirectories.begin(); itr != allDirectories.end(); ++itr){
        int currentSize1 = (*itr)->getSize();
        if(currentSize1 >= reduceAmount && currentSize1 < smallestAbove){
            smallestAbove = currentSize1;
        }
    }
    std::cout << smallestAbove << "\n";
}
