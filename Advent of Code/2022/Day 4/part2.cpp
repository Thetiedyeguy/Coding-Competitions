...
int main(int argc, char** argv){
    ...
    while(getline(file, line)){
        ...
        if((firstMin <= secondMin && firstMax >= secondMax) || (secondMin <= firstMin && secondMax >= firstMax) || (firstMin <= secondMax && firstMin >= secondMin) || (firstMax >= secondMin && firstMax <= secondMax)){
            count++;
            std::cout << "\n\n";
        }
    }
